"""Prospective work-order acquisition and paired sequential corrections."""
import argparse
import json
import os
...
def main():
    ...
    try:
        ...
        # Previously accepted routing task is calibration only, never pooled.
        from task import cases as routing_cases, A_WORDS, prompt as routing_prompt, score as routing_score
        calibration_path=Path('evidence/final-v1/ce-B-replay-128.safetensors')
        rt.restore(list(mx.load(str(calibration_path)).items()))
        calibration=[]
        for c in routing_cases(A_WORDS):
            guard();prefix=rt.encode(routing_prompt(c));r=rt.generate(prefix)
            calibration.append(dict(case=c,prefix=prefix,scores=routing_score(r['raw'],c),**r))
        save('routing-calibration.json',dict(path=str(calibration_path),sha256=sha(calibration_path),responses=calibration))
