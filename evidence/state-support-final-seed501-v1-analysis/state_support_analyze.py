...
def main():
 ...
 for state,rr in groups.items():
  ...
  if support:
   for label in ['current_support_identities','additional_familiar','fresh','acquired','target']:
    for scope,v in [('earlier_waiver',1),('newest_waiver',2)]:
     ss=[r for r in subsets[label] if t.oracle(r['case'],v)!=t.oracle(r['case'],v-1)]
     report[label+'_'+scope]=dict(n=len(ss),complete=sum(r['scores']['complete'] for r in ss))
   for source in ['correction','boundary','history']:
    keys={key(c) for src,c in t.schedule(support,2,64,'negative') if src==source}
    ss=[r for r in rr if key(r['case']) in keys]
